...
//	}
        if(mode == 'annual'){
                ...
                for(i=0; i<cycles.length; i++){
                        var option = document.createElement("option");
                        option.text = cycles[i].displayName;
                        option.value = cycles[i].name;
                        element.add(option);
                }
        }
...
//        }
                ...
                var element = document.getElementById("satellite");
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}

                for(i=0; i<satellites.length; i++){
                        var option = document.createElement("option");
                        option.text = satellites[i].displayName;
                        option.value = satellites[i].name;
                        element.add(option);
                }
        }
...
function getURL(annual,month,model,variable,cycle,satellite,frame){
	var newurl = url.replace("YYYY",annual);
        ...
        newurl = newurl.replace("CC",cycle);
        newurl = newurl.replace("SSS",satellite);
...
function showImage(){
	...
	//document.getElementById("valid").selectedIndex = frames.indexOf(parseInt(imageObj.frame));//(parseInt(imageObj.frame) / incrementFrame);
        ...
        document.getElementById("cycle").selectedIndex = searchByName(imageObj.cycle,cycles);
        document.getElementById("satellite").selectedIndex = searchByName(imageObj.satellite,satellites);
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		...
		var urls = getURL(obj.annual,obj.month,obj.model,obj.variable,obj.cycle,obj.satellite,i);		
...
//Change plot type from dropdown menu
...
// }
// Satellite
// Change cycle from dropdown menu
function changeCycle(id){
       imageObj.cycle = id;
       preload(imageObj);
       showImage();
       document.getElementById("cycle").blur();
}

function changeSatellite(id){
        imageObj.satellite = id;
        preload(imageObj);
        showImage();
        document.getElementById("satellite").blur();
}
