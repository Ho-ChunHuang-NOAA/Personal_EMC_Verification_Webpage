...
//	}
		...
		for(i=0; i<stats.length; i++){
			var option = document.createElement("option");
			option.text = stats[i].displayName;
...
function getURL(annual,month,model,stat,variable,day,validtime,domain,frame){
        var newurl = url.replace("YYYY",annual);
        newurl = newurl.replace("MM",month);
        newurl = newurl.replace("NNN",model);
        newurl = newurl.replace("SSS",stat);
	newurl = newurl.replace("FFF",variable);
        newurl = newurl.replace("DDD",day);
	newurl = newurl.replace("HH",validtime);
        newurl = newurl.replace("QQQ",domain);
	return newurl;
}
...
function showImage(){
	...
	//document.getElementById("valid").selectedIndex = frames.indexOf(parseInt(imageObj.frame));//(parseInt(imageObj.frame) / incrementFrame);
        ...
        document.getElementById("stat").selectedIndex = searchByName(imageObj.stat,stats);
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		...
		var urls = getURL(obj.annual,obj.month,obj.model,obj.stat,obj.variable,obj.day,obj.validtime,obj.domain,i);		
